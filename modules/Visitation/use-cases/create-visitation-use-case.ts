...
export class CreateVisitationUseCase {
    ...
    async execute({ houseId, status, visitControlMeasures, containers }: CreateVisitationUseCaseRequest) {
        ...
        const invalidTypes = visitControlMeasures.filter(measureType => !CONTROL_MEASURE_TYPE.includes(measureType))
        if(invalidTypes) {
        ...
        for (const visitControlMeasure of visitControlMeasures) {
            const createdVisitControlMeasure = await this.visitControlMeasureRepository.create({
        ...
        return {
            visitation: {
                ...visitation,
                controlMeasures: [ 
                    ...createdVisitControlMeasures
                ],
                containers: [
                    ...createdVisitContainers
                ]
            }
        }
    }
